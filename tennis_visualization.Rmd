...
# Loading in Data
```{r}
pointdata <- read.csv("GovindNanda_TylerZink_Point_ForVisuals.csv")
shotdata <- read.csv("GovindNanda_TylerZink_Shot.csv")
...
percent_table <- function(pointdata, player){
  server_order <- unique(pointdata$serverName)
  pointdata$serverName <- factor(pointdata$serverName, levels = server_order)
...
```{r}
doubles_ex <- read.csv("Timothy_Giac vs. Columbia 0_00 - 9_38 - Sheet1.csv")

library(zoo)
doubles_ex$serverName[doubles_ex$serverName == ""] <- NA
doubles_ex$serverName <- na.locf(doubles_ex$serverName)

serve_placement(doubles_ex, player = "Timothy Li")
```
