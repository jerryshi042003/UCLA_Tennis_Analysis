...
percent_table <- function(shotdata, pointdata, player){
  ...
  servicePointsWon <- shotdata %>% 
    filter(serverName == player) %>% 
    summarise("servicePointsWonPercent" = ((length(which(isPointEnd == 1 & pointWonBy == player))) /
                (length(which(isPointEnd == 1))))*100)

  returnPointsWon <- shotdata %>% 
    filter(serverName != player) %>% 
    summarise("returnPointsWonPercent" = ((length(which(isPointEnd == 1 & pointWonBy == player))) /
                (length(which(isPointEnd == 1))))*100)
...
serve_placement <- function(pointdata, player){
  ...
  #   filter(serverName == player & side == "Ad") %>% 
  ...
  #   filter(serverName == player & side == "Deuce") %>% 
